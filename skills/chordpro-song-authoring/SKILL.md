...
{start_of_verse}
[C]questa matt[Am]ina,
[Dm]al primo inco[G7]ntro con lo specchio
{end_of_verse}
...
All four headers are required. `{key: ...}` always uses common note names
(`C`, `F`, `Am`).

## Chord notation

- Chords are inline, in brackets, immediately **before** the syllable they
  fall on: `[C]questa matt[Am]ina` — never on their own `{comment:}` line.
- **Always author chords in common notation** (`C Dm E7 G7 Am`), whatever
  the songbook's language. The project config parses common names
  (`notes.system: common` in `chordpro-ukulele.json`); Italian chord names
  on a printed page are produced **at build time** via ChordPro transcoding
  (`--transcode=latin`, e.g. `make guitar-ita`), never stored in `.cho`
  files. `songbook.yaml`'s `notation:` field records what a songbook's
  files use — new files are always `common`; two legacy songbooks
  (`canzoni-ribelli`, `hbs-ita`) still store `latin`. Never mix notations
  within one songbook; converting a legacy songbook is its own task with
  user sign-off.
- Italian → common mapping (for reading Italian source sheets):
  DO=C, RE=D, MI=E, FA=F, SOL=G, LA=A, SI=B; `LA-`=Am, `SOL7`=G7,
  `RE-7`=Dm7. Slash chords keep both parts: `DO/SOL`=C/G.
- Preserve slash chords intact (`[D/F#]`, `[F/C]`); never split them.
...
- **Never put a `label="..."` attribute on a section block.** Labels print
  as their own line and eat vertical space; the layout is one song per
  page, so labels push songs onto a second page. Intro/Bridge/Outro/Solo
  are plain unlabeled verse blocks with inline chords:
  ...
  NOT `{comment: Intro}` followed by a bare chord line, and NOT
  `{start_of_verse label="Intro"}`. Files that still carry labels are
  legacy: don't add new ones; stripping existing ones is a separate `fix`
  commit.
...
| Put intro chords in a `{comment:}`                            | Plain `{start_of_verse}` block with inline `[CHORD]`s — no label                                                        |
| Label sections (`{start_of_verse label="Intro"}`)             | Never — labels consume page height; use plain blocks                                                                    |
| Source is an Italian sheet with `DO`/`SOL-` chords            | Transcribe to common notation (`C`/`Gm`) via the mapping above                                                          |
| Write `{key: Do}` for Italian songs                           | Key header is common notation: `{key: C}`                                                                               |
...
1. Read 1–2 sibling `.cho` files in the target songbook to confirm local
   conventions (RIT comment style, numbering, repeat markers). Chords are
   common notation regardless.
...
1. Wrap sections in paired verse/chorus markers — no `label=` attributes.
1. Verify:
   - every `start_of_*` has a matching `end_of_*` (grep both, counts equal);
   - no `{comment:}` line contains only chords;
   - no `label=` attribute on any section block;
   - all chords are common notation (no `DO`/`SOL`/`LA-` left over);
...
- **Chord-only lines inside a section** (instrumental figures): allowed as
  consecutive `[CHORD]` brackets on one line inside a plain verse block.
- **Unknown/exotic chords** (`G7sus2`, `Cm6`): valid in the file, but the
